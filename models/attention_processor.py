...
class AttnProcessor2_0:
    ...
    def __init__(self, ref_key_value_attn=False, **kwargs):
        self.ref_key_value_attn = ref_key_value_attn
        if not hasattr(F, "scaled_dot_product_attention"):
            raise ImportError("AttnProcessor2_0 requires PyTorch 2.0, to use it, please upgrade PyTorch to 2.0.")

    def copy_reference_feat(self, feat, video_length):
        # feat:[B*F,HW,C]
        feat = rearrange(feat, "(b f) hw c -> b f hw c", f=video_length)
        ref_feat = feat[:, :1].repeat(1, video_length, 1, 1)  # [B,1,HW,C]->[B,F,HW,C]
        feat = torch.cat([feat, ref_feat], dim=2)  # [B,F,HW*2,C]
        feat = rearrange(feat, "b f hw2 c -> (b f) hw2 c")
        return feat
    ...
    def __call__(
        ...
        if encoder_hidden_states is None or encoder_hidden_states.shape[1] > 1:
            query = attn.to_q(hidden_states, *args)
            ...
            # query,key,value:[B*F,HW,C]
            if self.ref_key_value_attn and encoder_hidden_states.shape[1] == hidden_states.shape[1]:  # is self-attn!
                key = self.copy_reference_feat(key, video_length=video_length)
                value = self.copy_reference_feat(value, video_length=video_length)
            ...
            hidden_states = hidden_states.to(query.dtype)
        else:  # cross-attn and the length of key and value is 1
            if attn.norm_cross:
                encoder_hidden_states = attn.norm_encoder_hidden_states(encoder_hidden_states)
            hidden_states = attn.to_v(encoder_hidden_states, *args).repeat(1, hidden_states.shape[1], 1)
