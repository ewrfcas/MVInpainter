...
class UNet3DConditionModel(ModelMixin, ConfigMixin, UNet2DConditionLoadersMixin, PeftAdapterMixin):
    r"""
    ...
    def __init__(
        ...
        self.transformer_in = TransformerTemporalModel(
            ...
            attention_head_dim=block_out_channels[0] // 8,
    ...
    def forward(
        ...
        Args:
            sample (`torch.FloatTensor`):
                The noisy input tensor with the following shape `(batch, channel, framerate, width, height)`.
    ...
    def from_pretrained_2d(cls, pretrained_model_name_or_path: Optional[Union[str, os.PathLike]],
                           rank=0, do_not_load_weights=False, **kwargs):
        ...
        if from_flax:
            ...
            low_cpu_mem_usage = False  # low_cpu_mem_usage is not support here

            model = cls.from_config(config, **unused_kwargs)

            if not do_not_load_weights:
                state_dict = load_state_dict(model_file, variant=variant)

                # reset some weights for Unet3D
                for k in state_dict:
                    if ("proj_in" in k or "proj_out" in k) and len(state_dict[k].shape) == 4 and state_dict[k].shape[2] == state_dict[k].shape[3] == 1:
                        # print(f"Reshape {k} from {state_dict[k].shape} to {state_dict[k].shape[:2]}")
