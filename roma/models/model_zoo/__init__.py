...
def roma_outdoor(device, weights=None, dinov2_weights=None, coarse_res=560, upsample_res=864, amp_dtype: torch.dtype = torch.float16):
...
def roma_indoor(device, weights=None, dinov2_weights=None, coarse_res = 560, upsample_res = 864, amp_dtype: torch.dtype = torch.float16):
    if isinstance(coarse_res, int):
        coarse_res = (coarse_res, coarse_res)
    if isinstance(upsample_res, int):    
        upsample_res = (upsample_res, upsample_res)
