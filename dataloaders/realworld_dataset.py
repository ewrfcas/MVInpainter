...
class RealWorldDataset(Dataset):
    def __init__(self, dataset_root, image_height, image_width, n_frames_per_sequence,
        ...
        scenes = glob(f"{dataset_root}/*")
        self.scenes = [s.split("/")[-1] for s in scenes]
        for scene in self.scenes:
            self.global_data_list[scene]["images"] = sorted(glob(f"{dataset_root}/{scene}/images/*"))
            self.global_data_list[scene]["masks"] = sorted(glob(f"{dataset_root}/{scene}/masks/*"))
            inpainted_list = glob(f"{dataset_root}/{scene}/{reference_path}/*")  # 读取编辑后的frame0
            if reference_num > 0:
                inpainted_list = inpainted_list[:reference_num]
            else:
                self.total_num += len(self.global_data_list[scene]["images"])
            self.total_num += len(self.global_data_list[scene]["images"]) * len(inpainted_list)
            self.global_data_list[scene]["inpainted"] = inpainted_list
    ...
    def realworld_reset_dataset(self, rank=0, **kwargs):
        ...
        for class_name in tqdm(class_names, desc=f"loading realset...", disable=rank != 0):
            ...
            if self.reference_split > 0:
                select_indices = np.arange(0, len(image_indices))
                for j in range(self.reference_split):
                    select_indices_ = select_indices[j::self.reference_split]
                    new_image_idx = np.array(image_indices)[select_indices_].tolist()[:self.n_frames_per_sequence]
                    new_mask_idx = np.array(mask_indices)[select_indices_].tolist()[:self.n_frames_per_sequence]
                    if kwargs.get("sort_frames", True):
                        new_image_idx.sort(key=lambda x: x.split("/")[-1])
                        new_mask_idx.sort(key=lambda x: x.split("/")[-1])
                    # 第0张临时替换为重复的图片，使用的时候会被替换为inpainted guidance
                    if len(new_image_idx) < self.n_frames_per_sequence:
                        new_image_idx = [new_image_idx[0]] + new_image_idx
                        new_mask_idx = [new_mask_idx[0]] + new_mask_idx
                    for inpainted in self.global_data_list[f"{class_name}"]["inpainted"]:
                        ...
                        fname_list.extend([f"split_{j}"] * len(new_image_idx))
    ...
    def __getitem__(self, index):
        ...
        if os.path.exists(f"{caption_path}/caption/{self.inpainted_list[index].split('/')[-1].split('.')[0]}.txt"):
            captions = []
            with open(f"{caption_path}/caption/{self.inpainted_list[index].split('/')[-1].split('.')[0]}.txt", "r") as f:
                for line in f:
            ...
            if self.caption_type is None or self.caption_type == "first":
                ...
                caption = captions[-1]
        elif os.path.exists(f"{caption_path}/caption.txt"):
            captions = []
