...
class Real10kDataset(Dataset):
    def __init__(self, dataset_root, data_list, image_height, image_width, n_frames_per_sequence,
        ...
        if mode == 'val':
            self.real10k_reset_dataset(mode=mode, rank=rank)

    def __len__(self):
        return len(self.data_list_per_epoch)

    def real10k_reset_dataset(self, mode="train", rank=0, **kwargs):
        file_list = []
        sequence_index = []
        ...
        for class_name in class_names: # tqdm(class_names, desc=f"loading real10k {mode} per epoch...", disable=rank != 0):
            seq_indices = self.global_seq_dict[class_name].copy()  # 首先获取某个场景图片数量
            seq_indices = [f"{self.prefix}/{class_name}/{s}" for s in seq_indices]
            splits = []
            st = 0
    ...
    def __getitem__(self, index):
        ...
        class_name = self.data_list_per_epoch[index].split("/")[-2]
