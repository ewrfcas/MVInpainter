...
class DL3DVDataset(Dataset):
    def __init__(self, dataset_root, data_list, image_height, image_width, n_frames_per_sequence,
        ...
        self.global_seq_dict = pickle.load(open("./data/dl3dv/global_seq_dict.pkl", "rb"))
        self.total_num = 0
        with open(data_list, "r") as f:
            for line in tqdm(f, desc=f"loading global dl3dv mixed {mode} sequences...", disable=rank != 0):
        ...
        if mode == 'val':
            self.dl3dv_reset_dataset(mode=mode, rank=rank)

    def __len__(self):
        return len(self.data_list_per_epoch)

    def dl3dv_reset_dataset(self, mode="train", rank=0, **kwargs):
        file_list = []
        sequence_index = []
        ...
        for class_name in class_names: # tqdm(class_names, desc=f"loading dl3dv mixed {mode} per epoch...", disable=rank != 0):
    ...
    def __getitem__(self, index):
        ...
        class_name = "/".join(self.data_list_per_epoch[index].split("/")[-3:-1])
