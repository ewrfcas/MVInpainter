...
class CO3Dv2Dataset(Dataset):
    def __init__(self, dataset_root, data_list, image_height, image_width, n_frames_per_sequence,
                 ...
                 **kwargs):
        super(CO3Dv2Dataset, self).__init__()
        ...
        self.global_seq_dict = pickle.load(open("./data/co3dv2/global_seq_dict.pkl", "rb"))
        self.total_num = 0
        with open(data_list, "r") as f:
            for line in tqdm(f, desc=f"loading global co3dv2 {mode} sequences...", disable=rank != 0):
                line = line.strip()
        ...
        if mode == "train":
            ...
            self.data_list_per_epoch = self.co3d_reset_dataset(mode=mode, rank=rank)

    def __len__(self):
        return len(self.data_list_per_epoch)

    def co3d_reset_dataset(self, mode="train", rank=0, **kwargs):
        file_list = []
        sequence_index = []
        ...
        for class_name in class_names:  # tqdm(class_names, desc=f"loading mvimagenet {mode} per epoch...", disable=rank != 0):
            ...
            for seq_name in seq_names:
                ...
                seq_indices = [f"{class_name}/{seq_name}/images/{l}.jpg" for l in seq_indices]
    ...
    def __getitem__(self, index):
        ...
        mask = cv2.imread(f"{self.dataset_root}/{class_seq_name}/masks/{filename.replace('.jpg', '.png')}")
        ...
        if self.image_height > image.shape[0]:
        ...
        try:
            if self.load_caption and self.caption_path is not None:
                caption_path = f"{self.caption_path}/co3dv2/{class_seq_name.split('/')[0]}/{class_seq_name.split('/')[1]}.txt"
